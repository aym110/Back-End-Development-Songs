...
@app.route("/count")
def CountSongs():
    collections = db.songs
    num_songs = collections.count_documents({})
    return jsonify(dict(count=num_songs)),200
...
def delete_song(id):
    song_delete = db.songs.delete_one({"id":id})
    if song_delete.deleted_count == 0:
        return {"message": "song not found"}, 404
    return  jsonify(),204
